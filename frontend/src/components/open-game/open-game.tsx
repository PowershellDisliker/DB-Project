...
function OpenGameComp({game}: OpenGameProps) {
    ...
    useEffect(() => {const inner = async () => {
        ...
        if (game.user_1_id) {
            const user: GetPublicUserResponse = await getPublicUser(config.BACKEND_URL, auth.token, game.user_1_id);
            console.log(user.username);
            setUser1Username(user.username);
        }

        if (game.user_2_id) {
            const user2: GetPublicUserResponse = await getPublicUser(config.BACKEND_URL, auth.token, game.user_2_id);
